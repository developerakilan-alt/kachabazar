...
const SearchScreenNew = ({
  ...
  const topCategories = flattenCategories(categories);
                  ...
                  >
                    <div className="space-y-0.5 pb-4 max-h-60 overflow-y-auto">
                      {categories?.filter(parent => {
                        const name = showingTranslateValue(parent?.name);
                        return name && name !== "Uncategorized";
                      }).map(parent => (
                        <div key={parent._id}>
                          <div className="flex items-center">
                            <button
                              onClick={() => handleCategoryClick(parent._id, parent.name)}
                              className={`flex-1 block rounded-lg px-3 py-2 text-left text-sm transition-all duration-150 ${
                            ...
                            {parent.children?.length > 0 && (
                              <button
                                ...
                                aria-label={expandedParents[parent._id] ? "Collapse" : "Expand"}
                              >
                          ...
                          {expandedParents[parent._id] && parent.children?.map(child => {
                            const childName = showingTranslateValue(child?.name);
                            if (!childName || childName === "Uncategorized") return null;
                            return (
                              <button
                                key={child._id}
                                onClick={() => handleCategoryClick(child._id, child.name)}
                                className={`block w-full rounded-lg pl-8 pr-3 py-1.5 text-left text-sm transition-all duration-150 ${
