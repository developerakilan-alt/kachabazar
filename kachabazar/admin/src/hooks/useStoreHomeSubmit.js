  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!isSave) {
        ...
        // Revalidate store cache so changes reflect immediately
        try {
          const storeUrl =
            import.meta.env.VITE_APP_STORE_URL ||
            settingData?.setting?.store_url;
          await fetch(
            `${storeUrl}/api/revalidate?tag=all&secret=hautecouturejewellery-revalidate-2024`,
          );
        } catch (e) {
    ...
    const getStoreCustomizationData = async () => {
      try {
        ...
        if (res) {
          ...
          setValue("twitter_handle", res.seo.twitter_handle);
          setValue("google_verification", res.seo.google_verification);
          setValue("robots", res.seo.robots);
        }
      } catch (err) {
        notifyError(err ? err?.response?.data?.message : err?.message);
      }
