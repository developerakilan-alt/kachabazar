...
const useFilter = (data) => {
  ...
  const serviceData = useMemo(() => {
    ...
    if (location.pathname === "/dashboard") {
      const orderPending = services?.filter(
        (statusP) =>
          statusP.status === "Pending" || statusP.status === "pending",
      );
      setPending(orderPending);
      const orderProcessing = services?.filter(
        (statusO) =>
          statusO.status === "Processing" || statusO.status === "processing",
      );
      setProcessing(orderProcessing);
      const orderDelivered = services?.filter(
        (statusD) =>
          statusD.status === "Delivered" || statusD.status === "delivered",
      );
  ...
  const handleSelectFile = (e) => {
    ...
    // Check for valid file types: JSON, CSV, XLS, XLSX
    const validTypes = [
      "application/json",
      "text/csv",
    ...
    if (file.type === "application/json" || file.name?.endsWith(".json")) {
      ...
      fileReader.onload = (e) => {
        try {
          ...
          // Handle different JSON structures - could be array or object with data property
          let text = Array.isArray(parsed)
            ? parsed
          ...
          if (!text || !Array.isArray(text)) {
            notifyError(
              "Invalid JSON format. File must contain an array of items.",
            );
            setFileName("");
            setIsDisable(false);
            return;
          ...
          if (text.length === 0) {
            notifyError("The file contains no data to import.");
            setFileName("");
            setIsDisable(false);
            return;
          }
          ...
          notifySuccess(`${data.length} items loaded from file`);
        } catch (error) {
          console.error("Error parsing JSON file:", error);
          notifyError("Invalid JSON file format. Please check your file.");
          setFileName("");
          setIsDisable(false);
        }
      };
      ...
      fileReader.onload = async (event) => {
        try {
          ...
          if (!json || json.length === 0) {
            notifyError("The CSV file contains no data to import.");
            setFileName("");
            setIsDisable(false);
            return;
          }
          ...
          if (location.pathname === "/categories") {
            data = json.map((value) => {
              return {
                ...
                id: value.id,
                status: value.status,
                name: value.name ? JSON.parse(value.name) : {},
                description: value.description
          ...
          if (location.pathname === "/attributes") {
            data = json.map((value) => {
              return {
                status: value.status,
                title: value.title ? JSON.parse(value.title) : {},
                name: value.name ? JSON.parse(value.name) : {},
                variants: value.variants ? JSON.parse(value.variants) : [],
                option: value.option,
                type: value.type,
              };
            });
          ...
          notifyError("Invalid CSV file format. Please check your file.");
          setFileName("");
          setIsDisable(false);
        }
      };
      ...
      // Unsupported file type - already checked at the start but this is a fallback
      notifyError("Please select a valid .JSON/.CSV/.XLS file first!");
    }
  };
